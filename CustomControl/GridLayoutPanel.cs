...
{
    public class GridLayoutPanel : TableLayoutPanel
    {
        [Browsable(false)]
        public new int ColumnCount { get; } = 16;

        //[Browsable(false)]
        public new int RowCount { get; set; } = 100;

        public GridLayoutPanel()
        {
            ControlAdded += GridLayoutPanel_ControlAdded;
        }


        private void GridLayoutPanel_ControlAdded(object sender, ControlEventArgs e)
        {
            if (e.Control.GetType().Equals(typeof(Label)))
            {
                e.Control.Anchor = System.Windows.Forms.AnchorStyles.Right;
            }
            else
            {
                e.Control.Anchor = 
